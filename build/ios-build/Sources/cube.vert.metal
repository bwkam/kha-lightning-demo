...
struct cube_vert_main_uniforms
{
    float4x4 model;
    float4x4 proj;
    float4x4 view;
...
struct cube_vert_main_in
{
    float3 aNormal [[attribute(0)]];
    float3 aPos [[attribute(1)]];
...
vertex cube_vert_main_out cube_vert_main(cube_vert_main_in in [[stage_in]], constant cube_vert_main_uniforms& uniforms [[buffer(1)]])
{
    cube_vert_main_out out = {};
    out.FragPos = float3((uniforms.model * float4(in.aPos, 1.0)).xyz);
    float4x4 _31 = transpose(spvInverse4x4(uniforms.model));
    out.Normal = float3x3(_31[0].xyz, _31[1].xyz, _31[2].xyz) * in.aNormal;
    out.gl_Position = (uniforms.proj * uniforms.view) * float4(out.FragPos, 1.0);
